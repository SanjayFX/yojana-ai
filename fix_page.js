...
// 1. Remove the skip link (Black bar)
code = code.replace(/<a href="#main" className="skip-link">Skip to main content<\/a>\s*/g, '');

// 2. Fix Hero whitespace
code = code.replace(
...
// Fix 4: Restore footer and add to the bottom.
const footerCode = `
            <div className="credits-footer mt-2">
              <span>
...
// Insert footer right before the end of the hero screen block
const endOfHeroScreen = `</section>
          </>
        )}`;
code = code.replace(endOfHeroScreen, \`</section>
\${footerCode}
          </>
        )}\`);

// Fix whitespace in steps section
code = code.replace(
  /<section className="py-16 px-4 sm:px-6 bg-white flex flex-col items-center">/,
  '<section className="pt-8 pb-12 px-4 sm:px-6 bg-white flex flex-col items-center mt-0">'
);
